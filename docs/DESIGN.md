...
[`tb_open`](../src/text_buffer.c#L2880) reads the front of the file and refuses
before discarding what is on screen, and
[`tb_load`](../src/text_buffer.c#L2751) has nothing to lose so it catches the
...
[`doc_stream()`](../src/text_buffer.c#L3021) walks HEAD, then memory, then what
